...
import { joinDigitRuns, normalizeWhitespace, parseAmount, type TraditionalExtractionResult } from "./shared";
...
function parseRows(normalized: string): RawRow[] {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    const reference = mobileMatch ? joinDigitRuns(mobileMatch[1]) : "";
...
export function extractOWealth(statementText: string): TraditionalExtractionResult {
