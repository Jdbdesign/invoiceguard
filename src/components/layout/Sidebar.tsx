...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-dvh w-60 flex-shrink-0 flex-col border-r border-[#2C2C2C] bg-[#141414] transition-transform duration-200 md:static md:z-auto md:translate-x-0 ${
      ...
      >
        <div className="flex items-center px-5 py-6">
          ...
          <img src="/auth/logo.svg" alt="Remitrak" className="h-6 w-auto" />
        </div>
              ...
              <Link
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition ${
                  active
                    ...
                    : "text-[#9A9A9A] hover:bg-[#1A1A1A] hover:text-[#B4B4B4]"
                }`}
              >
        ...
        <div className="border-t border-[#2C2C2C] px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#232323] text-xs font-semibold text-[#E5E5E5]">
              {initials}
            </div>
            <div className="min-w-0 leading-tight">
              <p className="truncate text-xs font-medium text-[#E5E5E5]">{userEmail}</p>
              <form action={logoutAction}>
                <button
                  type="submit"
                  className="text-[11px] text-[#6E6E6E] transition hover:text-[#B4B4B4]"
                >
                  Log out
                </button>
              </form>
