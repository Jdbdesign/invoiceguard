...
export function ShareLinkModal({
  ...
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    fetchJson<ShareLinkSummary[]>(`/api/clients/${clientId}/share-links`)
      .then((result) => {
        if (cancelled) return;
        setLinks(result);
      })
  ...
  async function handleGenerate() {
    ...
    try {
      const created = await fetchJson<
        ShareLinkSummary & { url: string }
      >(`/api/clients/${clientId}/share-links`, { method: "POST" });
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title={`Share ${clientName}`}>
        <div className="space-y-5">
          <p className="text-sm text-slate-600">
            Generate a read-only link anyone can open without an account — no edits,
            no deletions, just this client&apos;s invoices, payment plans, and activity.
            Links expire automatically after 30 days, or you can revoke one immediately.
          </p>
          ...
          <div>
            ...
            {loading ? (
              ...
              <p className="rounded-lg border border-dashed border-slate-200 py-6 text-center text-sm text-slate-500">
                No active share links for this client.
              </p>
            ) : (
